...
| `backend/openapi.json` | HTTP contract artifact | screen-MVP FastAPI/OpenAPI source of truth for generated clients |
...
| `frontend/lib/api/` | typed client surface | generated/maintained API contract consumer for screen-MVP routes |
| `frontend/` | mostly out of scope for PR1 | UI remains deferred; `frontend/lib/api/**` is allowed for contract/client work |
...
## Screen-MVP HTTP Slice Note
- The approved screen-MVP slice keeps FastAPI as the single HTTP source of truth.
- Contract artifacts for this slice live in `backend/openapi.json`.
- Frontend consumption artifacts for this slice live under `frontend/lib/api/**`.
- UI routes and components remain out of scope unless a later task explicitly widens that boundary.
